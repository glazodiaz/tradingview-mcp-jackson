...
const CONFIG = {
  symbol: process.env.SYMBOL || "TSLA",
...
async function fetchCandles(symbol, timeframe, limit = 100) {
  ...
  const alpacaTF = tfMap[timeframe] || "5Min";
  const url = `https://data.alpaca.markets/v2/stocks/${symbol}/bars?timeframe=${alpacaTF}&limit=${limit}&adjustment=raw`;
...
async function placeAlpacaOrder(symbol, side, sizeUSD, price) {
  ...
  if (qty < 1) throw new Error(`Trade size $${sizeUSD} too small to buy 1 share at $${price.toFixed(2)}`);
...
async function run() {
  ...
  console.log(`\nStrategy: ${rules.meta?.title || rules.strategy?.name || "Custom"}`);
